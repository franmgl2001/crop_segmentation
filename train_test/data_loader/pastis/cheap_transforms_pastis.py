...
class Cut:
    ...
    def __call__(self, sample):
        ...
        if self.seq_len > total_len:
            raise ValueError(f"Requested seq_len ({self.seq_len}) exceeds available length ({total_len}).")
...
class SimpleTransform:
    ...
    def __call__(self, sample):
        ...
        # Cut the inputs and DOY arrays
        cut_sample = self.cut(normalized_sample)
...
def process_image(image_id, seq_len=37, root_path="."):
    ...
    doy = number_dates_by_difference(feature['properties']['dates-S2'], start_date="20180917")
