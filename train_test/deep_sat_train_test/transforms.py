...
def PASTIS_segmentation_transform(model_config, is_training):
    ...
    if dataset_img_res != input_img_res:
        transform_list.append(
            Crop(
                img_size=dataset_img_res,
                crop_size=input_img_res,
                random=is_training,
    ...
    transform_list.append(
        TileDates(H=model_config["img_res"], W=model_config["img_res"], doy_bins=None)
    )  # tile day and year to shape TxWxHx1
    transform_list.append(
        CutOrPad(max_seq_len=max_seq_len, random_sample=False, from_start=True)
    )  # pad with zeros to maximum sequence length
    transform_list.append(
        UnkMask(unk_class=19, ground_truth_target="labels")
    )  # extract unknown label mask
...
class TileDates(object):
    ...
    def repeat(self, tensor, binned=False):
        if binned:
            out = (
                tensor.unsqueeze(1).unsqueeze(1).repeat(1, self.H, self.W, 1)
            )  # .permute(0, 2, 3, 1)
        else:
...
class Concat(object):
    ...
    def __call__(self, sample):
        ...
        sample = {
            key: sample[key] for key in sample.keys() if key not in self.concat_keys
        }
        return sample
...
class UnkMask(object):
    ...
    def __call__(self, sample):
        ...
        if "labels_grid" in sample.keys():
            sample["unk_masks_grid"] = self.rescale_2d_map(
                sample["unk_masks"].to(torch.float32), mode="nearest"
            ).to(torch.bool)

        return sample

    def rescale_2d_map(self, image, mode):
        img = image.unsqueeze(0).permute(
            0, 3, 1, 2
        )  # permute(2, 0, 1). put height and width in front
...
class AddBagOfLabels(object):
    ...
    def __call__(self, sample):
        ...
        bol[labels.unique().to(torch.long)] = 1.0
...
class AddEdgeLabel(object):
    ...
    def get_edge_labels(self, labels):
        ...
        lto = torch.nn.functional.pad(
            lto.unsqueeze(0).unsqueeze(0),
            [self.pad_size, self.pad_size, self.pad_size, self.pad_size],
            "reflect",
        )[0, 0]
        patches = lto.unfold(self.axes[0], self.nb_size, self.stride).unfold(
            self.axes[1], self.nb_size, self.stride
        )
        patches = patches.reshape(-1, self.nb_size**2)
        edge_map = (
            (patches != patches[:, 0].unsqueeze(-1).repeat(1, self.nb_size**2))
            .any(dim=1)
            .reshape(W, H)
            .to(torch.bool)
        )
...
class SOLOGroundTruths(object):
    ...
    def __call__(self, sample):
        ...
        ids_mask = torch.zeros(
            torch.Size([self.num_grid**2]) + ids.shape, dtype=torch.float32
        )  # [:2])
        cate_label = self.unk_class * torch.ones(
            [self.num_grid, self.num_grid], dtype=torch.int64
        )
        ...
        for ii, id_ in enumerate(uids):
            ...
            coord_w = int((center_w / ids.shape[1]) // (1.0 / self.num_grid))
            coord_h = int((center_h / ids.shape[0]) // (1.0 / self.num_grid))
            ...
            top = max(
                int(
                    round((center_h - self.sigma * dh) * self.num_grid / self.label_res)
                ),
                coord_h - 1,
            )
            down = min(
            ...
            left = max(
                int(
                    round((center_w - self.sigma * dw) * self.num_grid / self.label_res)
                ),
                coord_w - 1,
            )
            right = min(
            ...
            cate_label[top : (down + 1), left : (right + 1)] = labels[
                int(round(center_h)), int(round(center_w))
            ]
        ...
        ids_ind_masks[
            torch.arange(self.num_grid**2)[ids_mask.sum(dim=(1, 2, 3)) > 0]
        ] = True
